...
//added by Lucien
ros::Publisher pub_global_odom;
ros::Publisher pub_global_odom_2d;
...
void vio_callback(const nav_msgs::Odometry::ConstPtr &pose_msg)
{
    ...
    odometry.pose.pose.position.x = vio_t_cam.x();
    odometry.pose.pose.position.y = vio_t_cam.y();
    odometry.pose.pose.position.z = vio_t_cam.z();
    odometry.pose.pose.orientation.x = vio_q_cam.x();
    odometry.pose.pose.orientation.y = vio_q_cam.y();
    odometry.pose.pose.orientation.z = vio_q_cam.z();
    odometry.pose.pose.orientation.w = vio_q_cam.w();
    pub_global_odom.publish(odometry);
    ...
    transformStamped.child_frame_id = "global_camera";
    transformStamped.transform.translation.x = vio_t_cam.x();
    transformStamped.transform.translation.y = vio_t_cam.y();
    transformStamped.transform.translation.z = vio_t_cam.z();

    transformStamped.transform.rotation.x = vio_q_cam.x();
    transformStamped.transform.rotation.y = vio_q_cam.y();
    transformStamped.transform.rotation.z = vio_q_cam.z();
    transformStamped.transform.rotation.w = vio_q_cam.w();

    br.sendTransform(transformStamped);

    //publish the TF relationship between the world and the global optimized vio (2d)
     tf2::Quaternion q;
    q.setX(vio_q_cam.x());
    q.setY(vio_q_cam.y());
    q.setZ(vio_q_cam.z());
    ...
    //only consider the yaw angle, and to make sure the odometry is in line with the camera model
    double global_yaw = tf2::getYaw(q) + M_PI / 2;
    q.setRPY(0, 0, global_yaw);

    transformStamped.header.stamp = pose_msg->header.stamp;
    transformStamped.header.frame_id = "world";
    transformStamped.child_frame_id = "global_camera_2d";
    transformStamped.transform.translation.x = vio_t_cam.x();
    transformStamped.transform.translation.y = vio_t_cam.y();
    transformStamped.transform.translation.z = 0.0;  //vio_t_cam.z();
    ...
    pub_global_odom_2d.publish(odometry);
...
void command()
{
    ...
    while(1)
    {
        ...
        if (c == 'd')
        {
	        ...
	        pcl::io::savePCDFileASCII("/home/lucien_young/pcd_file_"+to_string(frame_index)+"keyframes.pcd", *(posegraph.save_cloud));
...
int main(int argc, char **argv)
{
    ...
    //added by Lucien
    pub_global_odom = n.advertise<nav_msgs::Odometry>("global_camera_odom", 1000);
    pub_global_odom_2d = n.advertise<nav_msgs::Odometry>("global_camera_odom_2d", 1000);
