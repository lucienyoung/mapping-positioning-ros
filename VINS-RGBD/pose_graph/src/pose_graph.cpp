...
void PoseGraph::loadKeyFrame(KeyFrame* cur_kf, bool flag_detect_loop)
{
    ...
    // if (!cur_kf->point_3d_depth.empty()){
    //     m_octree.lock();
    //     sensor_msgs::PointCloud2 tmp_pcl;
...
void PoseGraph::savePoseGraph()
{
    ...
    //pFile = fopen ("/home/lucien_young/output/pose_graph/pose_graph.txt","w");
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        fprintf (pFile, " %d %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %d %f %f %f %f %f %f %f %f %d %d\n",(*it)->index, (*it)->time_stamp, 
                                    ...
                                    (int)(*it)->keypoints.size(), (int)(*it)->point_3d_depth.size());
        ...
        for(int i = 0; i < (int)(*it)->point_3d_depth.size(); i++)
        {
            fprintf(densepoints_file, "%f %f %f\n", (*it)->point_3d_depth[i].x, (*it)->point_3d_depth[i].y, (*it)->point_3d_depth[i].z);
        }
                                    ...
                                    &keypoints_num, &densepoints_num) != EOF)
    {
        ...
        */
        ...
        for (int i = 0; i < densepoints_num; i++){
            double p_x, p_y, p_z;
            cv::Point3f pcl_tmp;
            if (!fscanf(densepoints_file, "%lf %lf %lf", &p_x, &p_y, &p_z)){
                printf("fail to load pose graph \n");
            }
            pcl_tmp.x = p_x;
            pcl_tmp.y = p_y;
            pcl_tmp.z = p_z;
            point_3d_depth.push_back(pcl_tmp);
        }
        fclose(densepoints_file);

        KeyFrame* keyframe = new KeyFrame(time_stamp, index, VIO_T, VIO_R, PG_T, PG_R, image, point_3d_depth, loop_index, loop_info, keypoints, keypoints_norm, brief_descriptors);
